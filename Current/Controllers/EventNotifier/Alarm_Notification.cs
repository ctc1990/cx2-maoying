            ...
            try
            {
                ...
                for (int i = 1; i < index.Count(); i++)
                {
                    if (e.NotificationHandle == hconnect[i - 1])
                    {
                        ...
                        {
                            SQL.Query.ExecuteNonQuery("update alarm_history set status = 0, recover_time = '" + dateTime + "' where (alarm_history.code = '" + alarm[0] + "' and alarm_history.io_code = '" + alarm[1] + "')", mySqlCommand);
                        }
